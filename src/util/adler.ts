...
export function adler32_walker(walker: ArrayBufferWalker, offset: number, length: number, seed?: number) {
    var a = 1, b = 0, L = offset + length, M = 0;
    if (typeof seed === 'number') { a = seed & 0xFFFF; b = (seed >>> 16) & 0xFFFF; }

    let endOffset = walker.offset;
    walker.offset -= length;

    while (walker.offset < endOffset) {
        let i = walker.offset;
        M = Math.min(L - i, 3850) + i;
        for (; i < M; i++) {
            a += walker.readUint8() & 0xFF;
            b += a;
        }
        a = (15 * (a >>> 16) + (a & 65535));
        b = (15 * (b >>> 16) + (b & 65535));
